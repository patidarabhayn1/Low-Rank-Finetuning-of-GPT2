...
class LoRALinear(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, rank) -> None:
        super().__init__()
        self.rank = rank
        self.L = nn.Parameter(torch.randn(in_dim, rank))
        nn.init.normal_(self.L, mean=0.0, std=1)
        self.R = nn.Parameter(torch.zeros(rank, out_dim))
        self.alpha = 1

    def forward(self, x):
        return x @ (self.L @ self.R) * self.alpha
...
class DistilRNN(nn.Module):
    ...
    def __init__(self, in_dim = 64, hidden_dim = 32, out_dim = 2) -> None:
        super().__init__()
        self.embed = nn.Embedding(GPTConfig.vocab_size, in_dim)
        self.rnn = nn.RNN(in_dim, hidden_size=hidden_dim, num_layers=1, nonlinearity='relu', dropout=0.1)
